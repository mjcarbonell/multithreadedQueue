...
int main(int argc, char **argv) {
  ...
  int tc = atoi(argv[1]);
  queue_t *q = queue_new(2); // QUEUE ONLY HAS 2 CAPACITIY/SIZE  
  if (q == NULL) {
    return 1;
  }
  pthread_t t1, t2[tc];
  ...
  for (int i = 0; i < tc; i++) {
    ...
    pthread_create(t2 + i, NULL, thread3, args2 + (2 * i));
    pthread_create(t2 + i, NULL, thread4, args2 + (2 * i));
  }
  printf("SIZE OF QUEUE %d\n", length(q));
  if(length(q) > 2){ // SINCE WE CALLED THREE THEADS TO SIMULTANEOUSLY PUSH AND OUR CAPACITY IS 2 THE LENGTH SHOULD NOT BE GREATER THAN 2... 
  	return 1;
